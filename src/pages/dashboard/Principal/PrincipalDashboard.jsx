...
//     { id: "students", label: "All Students", icon: "Graduation", badge: students?.length || null },
...
export default function PrincipalDashboard() {
  ...
  const { dark, toggleTheme } = useTheme();
  const { students, studentsLoading, loadStudents } = useStudents();
  const { teachers, teachersLoading, loadTeachers, addTeacher } = useTeachers(); // ✅ Add teachers hook
  const { users, createUser, filterRole, setFilterRole, search, setSearch } = useUsers();
  ...
  useEffect(() => {
    ...
    if (activeTab === "teachers") { // ✅ Add teachers condition
      loadTeachers();
    }
  ...
  const navItems = [
    { id: "profile", label: "My Profile", icon: "Profile" },
    { id: "teachers", label: "Teachers", icon: "Users", badge: teachers?.length || null }, // ✅ New Teachers tab
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 transition-colors duration-300">
      ...
      <main className="lg:ml-64">
        ...
        <div className="px-4 sm:px-6 py-6 sm:py-8">
          ...
          {activeTab === "teachers" && ( // ✅ New Teachers tab render
            <TeachersTab 
              teachers={teachers} 
